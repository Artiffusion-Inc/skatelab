...
class ProfileAuthFailureRawMessageReproTest {
    ...
    fun loadProfile_401_mustNotLeakRawHttpStatusText_repro() =
        testScope.runTest {
            ...
            val appError = state.appError
            assertNotNull("state must carry a typed AppError on failure", appError)
            org.junit.Assert.assertTrue(
                "A 401 must map to AppError.Auth so the UI can route to re-login, got: $appError",
                appError is AppError.Auth,
            )
        }
}
