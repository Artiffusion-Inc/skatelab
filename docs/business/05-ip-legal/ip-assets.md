...
|---------|-------------|-------|--------|
| SkatingVerse (28K клипов, 1 687 видео) | Public dataset | Research use | Скачан |
...
- **Объект:** Внешний вид датчиков IMU-трекеров, UI дизайн
...
- Биометрические данные (скелетон) → **отдельное согласие**, нельзя объединять с обычным. ⚠️ **Серая зона:** с 1.09.2025 (ФЗ-233) обезличенные данные можно обрабатывать без согласия для AI. Скелетон может квалифицироваться как биометрия **ИЛИ** как обезличенные данные — требует юридической консультации.
