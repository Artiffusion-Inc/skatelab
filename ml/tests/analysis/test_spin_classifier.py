...
def test_detect_spin_true():
    ...
    is_spin, duration, _hip_range = detect_spin(vel, hip_y, fps=30.0)
    assert is_spin
    assert duration > 1.0
...
def test_detect_spin_false():
    ...
    hip_y = np.ones(100) * 0.5
    is_spin, _, _ = detect_spin(vel, hip_y, fps=30.0)
    assert not is_spin
...
def test_detect_spin_too_short():
    ...
    hip_y = np.ones(100) * 0.5
    is_spin, duration, _ = detect_spin(vel, hip_y, fps=30.0)
    assert not is_spin
    assert duration < 1.0
...
def test_detect_spin_with_hip_variation():
    ...
    is_spin, _duration, hip_range = detect_spin(vel, hip_y, fps=30.0)
    assert is_spin
