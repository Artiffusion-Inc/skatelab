...
#547 fix: introduced UNSET sentinel to distinguish "field not provided"
...
def test_update_skips_unset_values():
    """#547: crud.session.update now uses UNSET sentinel to skip fields.

    Pre-fix: `if value is not None: setattr(session, key, value)` skipped
    None values, preventing intentional nulling.
    Post-fix: `if value is UNSET: continue` skips UNSET (sentinel
    ...
    # Post-fix: source uses the UNSET sentinel. Strip comments AND
    ...
    assert "value is UNSET" in source_no_strings, (
        "Expected update() to use the UNSET sentinel for skip-detection. "
    ...
    assert "value is not None" not in source_no_strings, (
        ...
        "with the UNSET sentinel — None is now a valid value to set."
    )
...
def test_update_can_null_field():
    ...
    Post-fix: the UNSET sentinel pattern allows callers to pass None
    to explicitly null a field, and UNSET to skip a field.
    """
    ...
    from app.crud.session import UNSET as session_unset  # type: ignore[attr-defined]
    ...
    # Sentinel pattern: passing UNSET skips the field
    ...
    assert session_dict2["error_message"] == "some error", (
        "Sentinel value (UNSET) is correctly skipped — old value persists"
    )


def test_user_update_has_same_pattern():
    """#547: crud.user.update now uses UNSET sentinel (same as session)."""
    import inspect
    import re
    ...
    source_no_strings = re.sub(r"#.*", "", source)
    source_no_strings = re.sub(r'""".*?"""', "", source_no_strings, flags=re.DOTALL)
    assert "UNSET" in source_no_strings, (
        "Expected user.update() to use the UNSET sentinel. "
        "Pre-fix: `value is not None` skipped None values silently."
    )
    assert "value is not None" not in source_no_strings, (
        "Pre-fix `value is not None` check should be replaced with UNSET"
    )
