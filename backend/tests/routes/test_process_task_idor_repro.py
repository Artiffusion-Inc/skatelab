...
async def test_process_status_idor_other_user_returns_403_repro(
    ...
    with patch("app.auth.ownership.get_task_state", new_callable=AsyncMock, return_value=state):
...
async def test_process_cancel_idor_other_user_returns_403_repro(
    ...
    with (
        patch("app.auth.ownership.get_task_state", new_callable=AsyncMock, return_value=state),
