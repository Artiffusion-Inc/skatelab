...
async def test_add_xp_concurrent_interleaving_lost_update_repro():
    ...
    # Both read the current level (total_xp=0) BEFORE either writes.
    ...
    # TxnA mutates + commits first (total_xp 0 → 100).
    level_a.total_xp += 100
    txn_a.add(level_a)
    await txn_a.flush()
    await txn_a.commit()

    # TxnB mutates based on its STALE read (total_xp=0 → 100) and commits,
    # overwriting TxnA's 100.
    level_b.total_xp += 100
    txn_b.add(level_b)
    await txn_b.flush()
    await txn_b.commit()
