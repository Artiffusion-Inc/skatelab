...
def tsx_nav(config: dict[str, Any], style: dict[str, str]) -> str:
    ...
    links_jsx = "\n          ".join(
        f'<a href="{link.get("url", "#")}" className="{style["muted"]} hover:{style["text"]} font-medium transition-colors">{link.get("text", "")}</a>'
...
def render_nav(config: dict[str, Any]) -> str:
    ...
    cta = config.get("nav_cta", {"text": "Get Started", "url": "#"})
    links = "\n".join(
        f'<li><a href="{escape(link.get("url", "#"))}">{escape(link.get("text", ""))}</a></li>'
        for link in nav_links
    )
    return f"""
...
def generate_html(config: dict[str, Any]) -> str:
    ...
    <footer><div class="container">
        <p>{escape(config.get("footer_text", str(datetime.now().year) + " " + config.get("brand", "Company") + ". All rights reserved."))}</p>
    </div></footer>""")
...
def main():
    ...
    if args.output:
        with Path(args.output).open("w") as f:
            f.write(output)
        print(f"Landing page written to {args.output}")  # noqa: T201
    else:
